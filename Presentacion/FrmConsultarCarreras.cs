...
namespace _1._1_Carreras.Presentacion
{
    public partial class FrmConsultarCarreras : Form
    {
        ...
        {
            if (MessageBox.Show("¿Está seguro que desea dar de baja la carrera " + lCarreras[lstCarreras.SelectedIndex].Nombre + " ?\n\n La carrera permanecera en la base de datos pero no estara activada", "Dar de Baja", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
               
                if (oBD.BorrarCarrera("SPBaja_Carrera", lCarreras[lstCarreras.SelectedIndex]) > 0)
                {
                    lstCarreras.SelectedIndex = -1;
                    MessageBox.Show("Carrera dada de baja", "Desactivada", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    CargarLista();
                    CantidadCarreras();
                }
                else
